...
if "remove_results" not in st.session_state:
    st.session_state.remove_results = {}  # keyed by hashed email+msgid

# -------------------- INPUTS --------------------
st.title("📬 Mandrill Email Debugger")
mandrill_api_key = st.secrets.get("MANDRILL_API_KEY", "")
email = st.text_input("Enter Customer Email").strip()
...
if st.button("Check Email Status"):
    ...
    if not is_valid_email(email):
        st.error("The email you entered doesn't look like a valid email address.")
        st.stop()
    ...
    # Filter for exact email address match (case-insensitive)
    ...
    # At this point we have at least one exact-match message
    st.subheader("📊 Email Summary")
    st.write(f"Total Exact-Match Emails Found: {len(exact_matches)}")
    status_counts = Counter([msg.get("state") for msg in exact_matches])
    ...
    for msg in exact_matches:
        ...
        # Determine card background depending on state
        if state == "sent":
            status_color = "#eafbea"  # greenish
            readable_state = "✅ Successfully delivered"
        elif state == "rejected":
            status_color = "#fdeaea"  # red-ish
            readable_state = "Rejected"
        else:
            status_color = "#fff8e5"  # bounce / soft-bounce fallback
            readable_state = state.capitalize()

        extra_html = ""

        # ---------- Delivered (nothing extra needed) ----------
        if state == "sent":
            pass  # already handled by readable_state

        # ---------- Rejected: show reason + remove from deny list ----------
        elif state == "rejected":
...
<p style='color:#721c24;background:#f8d7da;padding:10px;border-radius:5px;margin:5px 0;'>🚫 Rejected: {reject_reason}</p>"""
                ...
                with st.form(key=form_key):
                    ...
                    if submitted:
                        ...
                        if remove_response.status_code == 200:
                            result_json = remove_response.json()
                            if result_json.get("deleted"):
        ...
        # ---------- Bounced / other (including soft-bounced) ----------
        else:
            # Try to extract bounce info
            bounce_detail = (
                msg.get("diag", "") or
